...
include $core_config['apps_path']['plug'] . "/gateway/saicom/config.php";
...
switch (_OP_) {
	case "manage":
		...
		$tpl = array(
			...
			'vars' => array(
				...
				'Manage saicom' => _('Manage saicom'),
				'Gateway name' => _('Gateway name'),
				'Saicom send SMS URL' => _mandatory(_('saicom send SMS URL')),
				...
				'SAICOM_PUSH_DLR' => _('Remote gateway will push DLR and incoming SMS to your callback URL'),
				'BUTTON_BACK' => _back('index.php?app=main&inc=core_gateway&op=gateway_list'),
				'status_active' => $status_active,
				'saicom_param_url' => $plugin_config['saicom']['url'],
				'saicom_param_callback_url' => $plugin_config['saicom']['callback_url'],
				'saicom_param_callback_url_authcode' => $plugin_config['saicom']['callback_url_authcode'],
				'saicom_param_api_username' => $plugin_config['saicom']['api_username'],
				'saicom_param_module_sender' => $plugin_config['saicom']['module_sender'],
				'saicom_param_datetime_timezone' => $plugin_config['saicom']['datetime_timezone'] 
			) 
		);
		_p(tpl_apply($tpl));
		break;
	...
	case "manage_save":
		$up_url = ($_REQUEST['up_url'] ? $_REQUEST['up_url'] : $plugin_config['saicom']['default_url']);
		$up_callback_url = ($_REQUEST['up_callback_url'] ? $_REQUEST['up_callback_url'] : $plugin_config['saicom']['default_callback_url']);
		...
		if ($up_url) {
			...
			if (registry_update(0, 'gateway', 'saicom', $items)) {
		...
		header("Location: " . _u('index.php?app=main&inc=gateway_saicom&op=manage'));
		exit();
		break;
}
